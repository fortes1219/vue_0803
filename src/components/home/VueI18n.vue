			...
			<el-form style="width: 500px;">
				<el-form-item label="選擇語言">
					<el-select v-model="langSelect.select" :placeholder="$t('please_select')" clearable>
						<el-option v-for="(item, index) in langSelect.items" :key="index" :value="item.value" :label="item.label" />
					</el-select>
					<el-button type="primary" @click="changeLanguage(langSelect.select)">{{ $t('change_language') }}</el-button>
				</el-form-item>
			</el-form>
    </div>
  </div>
